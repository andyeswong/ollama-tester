...
<div class="container mx-auto px-4 py-6">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-2xl font-semibold text-gray-800">Edit Ollama Server</h1>
    </div>

    <div class="bg-white shadow-md rounded-lg overflow-hidden p-6">
        <form action="{{ route('ollama.servers.update', $server) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="url" name="url" id="url" value="{{ old('url', $server->url) }}" 
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 text-gray-700"
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" 
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50 text-gray-700">{{ old('description', $server->description) }}</textarea>
                @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
